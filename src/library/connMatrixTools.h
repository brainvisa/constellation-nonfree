...
namespace constel {
  ...
  void fillconnMatrix(
      Connectivities *conn, QuickMap &fiberExtremity1NeighMeshVertex,
      QuickMap &fiberExtremity2NeighMeshVertex, double connectivityThreshold,
      double distanceThreshold, unsigned connectionLength = 1);
  ...
  void fillconnMatrix(
      ...
      QuickMap &fiberExtremity2NeighMeshVertex, double connectivityThreshold,
      double distanceThreshold, std::size_t rowIndex_min,
      std::size_t rowIndex_max = 0, unsigned connectionLength = 1);
  ...
  void fillconnMatrixWithConnections(
      ...
      double connectivityThreshold, double distanceThreshold,
      std::size_t rowIndex_min = 0, std::size_t rowIndex_max = 0,
  ...
  void connMatrixSeedRegion(
      ...
      std::size_t seedRegionLabel, std::string connmatrix_filename,
      std::string connTexture_filename);

  void connMatrixSeedRegionSmoothed(
      const Fibers &fibers, const AimsSurfaceTriangle &inAimsMesh,
      Motion motion, const TimeTexture<short> &seedRegionsTex,
      std::size_t seedRegionLabel, float distthresh, float wthresh,
      std::string connmatrix_filename, std::string connTexture_filename = "",
