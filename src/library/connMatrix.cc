...
//             }
  ...
  {
    if (verbose) std::cout << "Start connMatrixRegionExtract" << std::endl;
...
//       std::cout << "i:" << i << std::endl;
      ...
      if (seedTex0[i]==seedRegionLabel)
      {
        ...
        extractConnMatrix[vertexCount] = AllMeshconnMatrixToAllMesh[i];
...
//       std::cout << "i:" << i << std::endl;
    ...
    bool verbose )
  {
    ...
    */
    ...
    const Texture<short> & targetTex0 = targetRegionsTex.begin()->second;
    size_t targetMeshVertexNb = connMatrixSeedMeshToTargetMesh[0].size();//number of columns
    size_t seedMeshVertexNb = connMatrixSeedMeshToTargetMesh.size();//number of rows
    if (verbose)
      cout << "seedMeshVertexNb:" << seedMeshVertexNb << endl;
...
//     std::cout << "five_count" << five_count << std::endl;
    for( size_t i = 0; i < seedMeshVertexNb; ++i )//iteration on the rows
    {
//       std::cout << "i" << i << std::endl;
//       if (i % five_count == 0 and five_count != 0)
//       {
...
//         std::cout << "j" << j  << std::endl;
        ...
        {
//           std::cout << "coucou1" << std::endl;
          connMatrixSeedMeshToTargetMeshTargets[i][targetLabel-1] += connMatrixSeedMeshToTargetMesh[i][j];
        }
      }
    }
    if (verbose)
  ...
  Connectivities * connMatrixNormalize( Connectivities * inConnMatrix_ptr,
                                        bool verbose )
  {
    if (verbose) std::cout << "Normalize connectivity matrix " << std::flush;
    Connectivities & inConnMatrix = *inConnMatrix_ptr;//inConnMatrix[i]= ith connectivity profile
    std::size_t colNb = inConnMatrix[0].size();
    std::size_t rowsNb = inConnMatrix.size();
...
//         }
//       }
      Connectivity line_i = inConnMatrix[i];
      if (line_i.is_null()!=1)
      {
        double norm_i = sqrt(til::dot<double>(line_i, line_i));
//         double sum_i = til::dot<double>(line_i, sparceVectorIdentity);
//         std::cout << ", norm_i:" << norm_i <<", sum_i:" << sum_i <<std::endl;
        if(norm_i!=0)
        {
          for(std::size_t j = 0; j < colNb; ++j)
          {
            ...
            inConnMatrix[i][j] = (1/norm_i)*connval;
          }
        }
      }
    }
    if (verbose) std::cout << "Done." << std::endl;
    return inConnMatrix_ptr;
  }//connMatrixNormalize
