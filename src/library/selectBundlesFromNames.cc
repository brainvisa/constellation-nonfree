...
namespace constel {
  ...
  //---------------------------------------------------------------------------
  SelectBundlesFromNames::SelectBundlesFromNames(
      vector<string> &select_bundles_name, bool verbose, bool as_regex,
      bool use_fiber_names) :
  ...
  //  filterBundleName
  //--------------------
  bool filterBundleName(
      const string &name, const set<string> &select_bundles_name,
      bool as_regex, set<regex_t *> &regex) {
    ...
    if (as_regex) {
      ...
      if (status == 0) bundle_selected = true;
      else bundle_selected = false;
    } else {
      // search bundle name
      set<string>::iterator recherche = select_bundles_name.find(name);
  ...
  //---------------------------------------------------------------------------
  void SelectBundlesFromNames::bundleStarted(
    ...
    _bundle_selected = filterBundleName(bundleInfo.name(),
    ...
    if (!_bundle_selected) {
      ...
      if (_verbose)
        cout << "bundle name: " << bundleInfo.name() << "... OK" << endl;
    }
  }

  //---------------------------------------------------------------------------
  void SelectBundlesFromNames::bundleTerminated(
      const BundleProducer &, const BundleInfo &bundleInfo) {
  ...
  //---------------------------------------------------------------------------
  void SelectBundlesFromNames::fiberStarted(
      const BundleProducer &, const BundleInfo & bundleInfo,
  ...
  //---------------------------------------------------------------------------
  void SelectBundlesFromNames::fiberTerminated(
      const BundleProducer &, const BundleInfo &bundleInfo,
      const FiberInfo &fiberInfo) {
    if (_bundle_selected) {
      if (_use_fiber_names) {
  ...
  //---------------------------------------------------------------------------
  void SelectBundlesFromNames::newFiberPoint(
      const BundleProducer &, const BundleInfo & bundleInfo,
