...
def remove_labels(tex, labels):
    ...
    Args:
        tex (aims time texture S16): labeled texture between 1 and max.
        labels (list): labels to remove        
    Returns:
...
def clean_gyri_texture(mesh, gyri_tex):
    ...
    Args:
        mesh (aims time surface): white mesh associated to gyri_tex
        gyri_tex (aims time texture S16): gyri texture 
                                          as full FreeSurfer parcellation.
    Returns:
        gyri_tex (aims time texture S16): new gyri texture, 
                                          without isolated vertex.
    
    """
    mesh_neighbors_vector = aims.SurfaceManip.surfaceNeighbours(mesh)
    cc_tex, areas_measures = MTT.connectedComponents(mesh, gyri_tex, 
                                 areas_mode = 1)
    wrong_labels = []
    ...
    for label in wrong_labels:
        ...
        for l in range(1, cc_nb):
            gyri_tex, win = change_wrong_labels(l + 1, label, gyri_tex, 
...
def texture_time(k_max, clusters_id, vertices_mesh, vertices_patch, mode, 
    ...
    Args:
        k_max (int): number of clusters.
        clusters_id (list): list of arrays. Each array is used to store 
                            the cluster number to which each item 
                            was assigned by the clustering algorithm.
        vertices_mesh (long): number of vertices for a given mesh.
        vertices_patch (list): set of vertices for a given patch.
        mode (int):
            1 -- average.
            2 -- concatenate.
    Returns:
        tex (aims time texture S16): for each time step, 
                                     results of the k-th clustering.
        
    """
    count = 0
    tex = aims.TimeTexture_S16()
    for k in range(k_max):
        tex[count].resize(vertices_mesh, 0)
        if mode == 1:
            tex[count].arraydata()[vertices_patch] = \
...
def add_texture(tex, add_tex, add_value):
    ...
    Args:
        tex (aims time texture S16): patch texture 1, fixed time-step.
        add_tex (aims time texture S16): patch texture 2, fixed time-step.
        add_value (int): value added to the original numbering.
    Returns:
