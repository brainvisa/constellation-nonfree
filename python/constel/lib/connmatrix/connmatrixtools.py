...
def generate_uniform_matrix(features):
    ...
    """
    n_samples, n_dimension = features.shape
    vector_a = (features[:, 0].max() - features[:, 0].min()) * numpy.random.random_sample((n_samples, 1)) + features[:, 0].min()
    vector_b = (features[:, 1].max() - features[:, 1].min()) * numpy.random.random_sample((n_samples, 1)) + features[:, 1].min()
    uniform_matrix = numpy.hstack((vector_a, vector_b))
    for i in range(2, n_dimension):
        ni = (features[:,i].max() - features[:, i].min()) * numpy.random.random_sample((n_samples, 1 ) ) + features[:, i].min()
...
def compute_mclusters_by_nbasins_matrix(reducedmatrix, parcels, timestep = 0, mode = "meanOfProfiles"):
  ...
  for i in xrange(len(labels_unique)):
    ...
    label_connMatrixToTargets_array = reducedmatrix[numpy.where(labels==label)]
...
def contingency_matrix( labels1, labels2 ):
  ...
  n = len( classes )
  contingency_matrix = numpy.array( [ zip(labels1, labels2 ).count(x) for x in itertools.product( classes, repeat = 2 ) ] ).reshape( n, n )
  #contingency_matrix = numpy.bincount( n * ( labels1 - 1 ) + ( labels2 - 1 ), minlength = n * n).reshape( n, n )
